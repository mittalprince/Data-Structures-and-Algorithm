...
2. jth item place some other index, but dont know on which index, so now total rem dearrangement exsist is (n-1), 
   as now I am sure only for ith item, which I placed on jth, but dont know aboyt jth item, so call on f(n-1)  
*/

/*
total ways = possive ways to place 1 item * possible ways of place (n-1) items
for 1 item its (n-1) and we have two possbility so we add them and multiply them
hence dp[n] = ((n-1)*(solve(n-1)+solve(n-2)))%mod;
*/
...
ll solve(int n){
    ...
    dp[n] = ((n-1)*(solve(n-1)+solve(n-2)))%mod;
    return dp[n];
}
