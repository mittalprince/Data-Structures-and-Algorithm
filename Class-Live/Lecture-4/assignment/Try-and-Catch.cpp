...
int main(){
    ...
    string throw_type="";
    int after_throw_try_catch_no = 0, before_throw_try_catch_n=0;
    for(int i=0; i<n; i++){
        getline(cin, s);
        s = removeSpace(s);
        ...
        if(!throw_occur){
            if(checkType(s, "try")){
                // stk.push(s);
                before_throw_try_catch_n++;
            }
            else{
                if(checkType(s, "catch")){
                    // if(!stk.empty()) stk.pop();
                    if(before_throw_try_catch_n) before_throw_try_catch_n--;
                }
                else{
                    throw_occur=true;
