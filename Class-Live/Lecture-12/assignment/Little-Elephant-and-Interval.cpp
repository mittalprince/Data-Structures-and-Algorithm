...
ll dp[20][10][10][2][2];
// state -> pos, first, last, tight, start
string s;
...
/*
ll solve(int pos, int first, int last, int tight){
    if(pos == s.length()){  
        return first == last;
    }

    if(dp[pos][first][last][tight] != -1) return dp[pos][first][last][tight];

    ll ans=0;
    int end = ((tight)?(s[pos]-'0'):9);
    for(int i=0; i<=end; i++){
        if(first == 0){
            res += solve(pos+1, i, i, tight&&(i==end));
        }
        else res += solve(pos+1, first, i, tight&&(i==end));
    }
