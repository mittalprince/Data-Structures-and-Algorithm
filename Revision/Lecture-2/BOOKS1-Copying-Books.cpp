...
void PrintConfiguration(ll books[], ll total_books, ll total_students, ll max_pages){
    ll t=0;
    vector<bool>slash(total_books, false);
    total_students--;

    for(ll i=total_books-1; i>=0; i--){
        if(t+books[i] <= max_pages and i>=total_students){
            t += books[i];
        }
        else{
            slash[i]=true;
...
void solve(){
    ...
    // cout<<ans<<endl;
    PrintConfiguration(books, no_of_books, no_of_students, ans);
}
